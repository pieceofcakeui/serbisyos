...
try {
    $stmt = $conn->prepare("INSERT INTO messages (sender_id, receiver_id, message, attachment, client_message_id)
                            VALUES (?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    $response = [
        ...
        'is_image' => !empty($attachment_url) && preg_match('/\.(jpg|jpeg|png|gif|webp)$/i', $attachment_url)
    ];
